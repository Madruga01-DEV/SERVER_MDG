...
function InventoryService.reloadInventory(player, id, type, source)
	...
	local totalWeight = 0
	for n,infos in pairs(itemList) do
		totalWeight = totalWeight + ((infos.weight or 0.0)*infos.count)
	end

	local payload = {
		totalWeight = totalWeight,
		useWeight = (invData.weight or 0.0) > 0.0,
		itemList = itemList,
...
function InventoryService.canStoreItem(identifier, charIdentifier, invId, name, amount)
	...
	local itemList = {}
	for weaponId, weapon in pairs(UsersWeapons[invId]) do
		if invData:isShared() or weapon.charId == charIdentifier then
			itemList[#itemList + 1] = Item:New({
				id            = weaponId,
				count         = 1,
				name          = weapon.name,
	...
	if invData:useWeight() then
		local item = exports.vorp_inventory:getItemByName(source, name)
		-- local item = SvUtils.FindItemByNameAndMetadata(invId, identifier, name, nil)
		local currentWeight = totalWeight

		if (currentWeight + ((item.weight or 0.0) + amount)) > invData.weight then
			return false, "Inventory limit weight"
		end
	end
...
function InventoryService.DiscordLogs(inventory, itemName, amount, playerName, type)
	...
	local webhook = CustomInventoryInfos[inventory].getWebhook and CustomInventoryInfos[inventory]:getWebhook() or ""
	if type == "Move" then
