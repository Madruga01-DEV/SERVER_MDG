...
function send_values() {
    if (document.getElementById("text_input").value === "") {
        ...
        $.post('http://mdgApi/button_1', JSON.stringify({ text: "close" }));
    } else {
        $.post('http://mdgApi/button_2', JSON.stringify({ text: document.getElementById("text_input").value }));
        document.getElementById("form_id").reset();
    }
    inInput = false;
}
...
function yes_func() {
    document.getElementById("form_id").reset();
    $.post('http://mdgApi/yes', JSON.stringify({ text: "yes" }));
    inAnswer = false
}

function no_func() {
    document.getElementById("form_id").reset();
    $.post('http://mdgApi/no', JSON.stringify({ text: "no" }));
    inAnswer = false
}
