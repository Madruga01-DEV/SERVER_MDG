    ...
    while true do
		...
		if hit then
			...
			if targetEnable == true then
				optimize = mdgApi.fpsTimer()
			end
		end
		if #points ~= 0 then
			optimize = mdgApi.fpsTimer()
			local coords = GetEntityCoords(PlayerPedId())
			mdgApi.IsPlayerInsidePolygon(coords, points, true)
...
function mdgApi.getFolder()
	return Config.FolderWithIcons
end

function mdgApi.drawMe(x, y, z, text, dist, marker)
...
function mdgApi.playerNearCoords(x, y, z, dist, mark)
...
function mdgApi.checkPlayersNearEntity(entity, distance)
...
function mdgApi.groundCheck(x, y)
...
function mdgApi.createBlip(text, x, y, z, sprite)
...
local lastVehicle = 0
function mdgApi.createVehicle(model, x, y, z, h)
...
function mdgApi.getLastVehicle()
    return lastVehicle
end
...
function mdgApi.showPrompt(name, prompt, show)
	if show == false then
...
function mdgApi.createPrompt(text, key, promptHash, holdMode, event)
...
function mdgApi.promptDelete(text, promptHash)
	if buttons[text..promptHash] ~= nil then
...
function mdgApi.setPromptVisible(text, hash, state)
	if state then
...
function mdgApi.getPrompt(promptHash, text)
				...
				if allButtonsPrompts[a] ~= nil then
					for c,d in pairs(allButtonsPrompts[a]) do
						if Citizen.InvokeNative(0x305C8DCD79DA8B0F, 0, d[3]) and Citizen.InvokeNative(0x0D00EDDFB58B7F28, d[1]) and d ~= nil and d[2] ~= nil then
							mdgApi.setPromptEnable(d[1], a, false)
							TriggerEvent(d[2])
						end
					end
				end
...
function mdgApi.getPromptButton(text, promptHash)
...
function mdgApi.setPromptEnable(text, hash, state)
	if state == true then
...
function mdgApi.getPromptButtonData(text, promptHash)
	local isComplete = false
	local button = buttons[text..promptHash]
	return button
end

function mdgApi.getTarget()
    return coords, entity, coordsTarget
end

function mdgApi.createPed(model,x,y,z,h,dist,scenario)
...
Citizen.CreateThread(function()
	while true do
		...
		for a,b in pairs(allNPCData) do
			if mdgApi.playerNearCoords(b.x, b.y, b.z, b.dist) then
		...
		for a,b in pairs(allNPCSpawned) do
			...
			if not mdgApi.playerNearCoords(npcCoords.x, npcCoords.y, npcCoords.z, b[1]) then
				deleteObj(a, b[2])
			end
		end
...
function mdgApi.getAllPeds()
	return GetGamePool('CPed')
end

function mdgApi.getAllEntities()
	return GetGamePool('CObject')
end

function mdgApi.getAllVehicles()
	return GetGamePool('CVehicle')
end

function mdgApi.getTime()
	return GetClockHours(), GetClockMinutes()
end


function mdgApi.playAnim(entity, dict, name, flag, time)
...
RegisterNetEvent('mdg_notify:notify')
AddEventHandler('mdg_notify:notify', function(title, subtext, icon, duration)
    show_notification(true, title, subtext, icon, duration)
end)

function mdgApi.notify(title, text, img, time)
	TriggerEvent("mdg_notify:notify", title, text, img, time)
end
...
function mdgApi.getAnswer(title, first, second, cb)
...
function mdgApi.getInput(title, subtext, cb)
...
function RayCastGamePlayCamera(distance)
	...
	if targetEnable == true then
		mdgApi.drawMe(destination.x, destination.y, destination.z, ".", 10.0)
	end
...
function mdgApi.IsPlayerInsidePolygon(playerPos, polyPoints, debug, issmall)
    ...
    for i = 1, #polyPoints do
        if debug then
            ...
            mdgApi.drawMe(polyPoints[i].x, polyPoints[i].y, polyPoints[i].z + 1.0, label, 4.0)
...
function mdgApi.calculatePolygonLength(polyPoints)
    local length = 0.0
...
RegisterNetEvent("mdg:SelectedCharacter")
AddEventHandler("mdg:SelectedCharacter", function(charid)
    Citizen.Wait(2000)
    TriggerServerEvent("mdgApi:checkLogin")
end)
...
RegisterNetEvent('mdgApi:createStash')
AddEventHandler('mdgApi:createStash', function(id, size)
	TriggerEvent("inventory:client:SetCurrentStash", id)
	TriggerServerEvent("inventoryOpenInventory", "stash", id, { maxweight = size*1000, slots = invSlots, })
end)
RegisterNetEvent('mdgApi:openStash')
AddEventHandler('mdgApi:openStash', function(id,size)
